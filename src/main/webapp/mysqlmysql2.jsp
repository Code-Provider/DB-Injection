...
<form method = "post" action = "mysqlmysql2mapfields">
<table class="table">
  <thead class="thead-dark">
    <tr>
      <th scope="col">MySQL tables</th>
      <th scope="col">MySQL2 tables</th>
  ...
  <c:forEach var = "mysqltable" items = "${mysqltables}">
    <tr id = "${count}" value ="workplz">
      ...
      <th scope="row" id = "th${count}" value = "${mysqltable}"><c:out value="${mysqltable}"/></th>
      
      <td class = "tddd"><select class="custom-select" id = "td${count}">
      <option value = "No Mapping">No Mapping</option>
      <c:forEach var = "mysql2table" items = "${mysql2tables}">
      <option value = "${mysql2table}"><c:out value="${mysql2table}"/></option>
      </c:forEach>
